...
class Manager extends EventEmitter {
  ...
  async updatePresence() {
     ...
     */
    await this.checker.wait(500); // eslint-disable-line no-magic-numbers
    if (!this.checker.growtopiaOpen) return this.manager.emit("debug", "[Manager]: Cannot update RPC presence data, growtopia is not running.");
    let presenceData = "";
    ...
    this.emit("debug", `[Manager]: Sending acitivty set request with the payload: ${JSON.stringify(presencePayload)}`);
    this.client.request("SET_ACTIVITY", presencePayload)
      .catch(this.handleError)
      .then(payload => this.emit(`[Manager]: Discord responded with payload: ${JSON.stringify(payload)}`));
    return true;
  }
