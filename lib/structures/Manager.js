...
class Manager extends EventEmitter {
  ...
  async updatePresence(resetTimestamp = true) {
    ...
    this.client.request("SET_ACTIVITY", presencePayload)
      ...
      .then(payload => this.emit(`[RPC]: Discord responded with payload: ${JSON.stringify(payload)}`));
    return true;
  }
  ...
  handleGrowtopiaClose() {
    ...
    this.manager.client.request("SET_ACTIVITY", {
      pid: process.pid,
      presence: {}
    }).then(payload => `[RPC]: Discord replied with the payload: ${JSON.stringify(payload)}`);
