...
class Growtopian extends EventEmitter {
  constructor(manager) {
    ...
    watchFile(path.join(this.dataPath, "save.dat"), () => {
      this.manager.emit("debug", "[Growtopian]: Save.dat file has been updated.");
  ...
  generatePresenceJson() {
    ...
    const worldName = (this.jammed ? "JAMMED!" : getSaveDatItem(this.dataPath, "lastworld").toUpperCase()) || "EXIT";
